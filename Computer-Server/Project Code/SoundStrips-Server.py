...
rms = 4500
preset_dict = {
    "Red": "30,100,25,255,0,0",
    "Green": "30,100,25,0,255,0",
    "Blue": "30,100,25,0,0,255"
}
saved_settings = []
device = 0
...
def load_preset():
    ...
    volume_slider.set(settings_list[0])
    brightness_slider.set(settings_list[1])
    fade_slider.set(settings_list[2])

    global color
    color = ((settings_list[3], settings_list[4], settings_list[5]),
             rgb_to_hex((int(settings_list[3]), int(settings_list[4]), int(settings_list[5]))))
    global color_button
...
volume_slider = Scale(customize_frame, label="Volume Threshold", from_=1, to=100, orient=HORIZONTAL, length=150,
                      bg='grey20', fg='White', troughcolor='Grey15', highlightthickness=0, bd=0)
volume_slider.set(30)
volume_slider.grid(row=0, column=2, columnspan=2, padx=(5, 0))
...
def add_preset(name):
    if name in preset_dict:
        ...
        preset_dict[name] = str(volume_slider.get()) + "," + str(brightness_slider.get()) + "," + str(fade_slider.get())
        for i in range(3):
...
github_button = Button(about_frame, text="Github Link", command=lambda: open_link("rm"), font=('Malgun Gothic', 10),
                       bg='grey15', fg='White', padx=5, pady=5, bd=0, activebackground='grey10').grid(row=2, column=0)
readme_button = Button(about_frame, text="README", command=lambda: open_link("glp"), font=('Malgun Gothic', 10),
                       bg='grey15', fg='White', padx=5, pady=5, bd=0, activebackground='grey10').grid(row=2, column=1)
license_button = Button(about_frame, text="LICENCE", command=lambda: open_link("git"), font=('Malgun Gothic', 10),
...
def process_audio():
    ...
    while power_on:
        ...
        volume_level = volume_slider.get() * (15000/100)
        if rms > volume_level:
            ser.write(1)
